...
- **User 1**: Located the "Draw a Marker" button and used it to set a pin. He then filled in the Obstacles type, a short description and then a height
...
### 1. Install Siege
If you don't have Siege installed, you can do so using Homebrew:
```bash
brew install siege
```

### 2. Create `spike.siege` and `maxload.siege` File
Create two new files called `spike.siege` and `maxload.siege` in the root directory of the project, and copy the following content:
```bash
/NaviSafe.Tests/StressTests/spike.siege
/NaviSafe.Tests/StressTests/maxload.siege
```

After you have created these files, open them in your favorite text editor and replace the contents with the following:
```bash
# Spike Load Test
...
### 3. Create `urls.txt` File 
Create a new file called `urls.txt` in the root directory of the project, and copy the following content:
```bash
# GET Endpoints (Both roles)

https://localhost:8081/
https://localhost:8081/Account/Login
https://localhost:8081/Account/Register
...
### 4. Run the Tests 
To execute the tests, either use **Windows PowerShell** or **Terminal (Mac)**, navigate to the project root directory, and run the following commands:
```bash
# Run the tests in parallel
siege -f spike.siege -c 30 -t 30s
siege -f maxload.siege -c 200 -t 120s
```

**-c** specifies the number of concurrent users, and **-t** specifies the duration of the test.
